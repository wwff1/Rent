...
export default function TypePage() {
    ...
    async function getID(id) {
        ...
        if (response.ok === true) {
            const type = await response.json()
            setForm({ ...form, id: type._id, name: type.name })
            id_edit = type._id;
        }
    }
    ...
    async function EditType(typeId, typeName) {
        const response = await fetch("api/type/edit", {
            method: "PUT",
    ...
    function row(type) {
        ...
        if(document.querySelector("tr[data-rowid='" + type._id + "']"))
        {
            document.querySelector("tr[data-rowid='" + type._id + "']").remove()
        }

        const tr = document.createElement("tr");
        tr.setAttribute("style", "padding:10px;");
        tr.setAttribute("data-rowid", type._id);

        const nameTd = document.createElement("td");
        nameTd.setAttribute("style", "padding:5px; text-align: center");
        nameTd.append(type.name);
        tr.append(nameTd);
        ...
        editLink.setAttribute("data-id", type._id);
        ...
        removeLink.setAttribute("data-id", type._id);
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={6}>
                            <Paper className={classes.paper}>
                                <Title>Список типов кузовов автомобилей</Title>
                                <table  >
                                    <thead>
                                    <tr>
                                        <th>Тип кузова</th>
